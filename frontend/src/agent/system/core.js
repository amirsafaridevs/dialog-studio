...
Workspace = the active WordPress child theme (its path is in your context block). You may READ anything under the WordPress install (the child theme, other plugins, and core). You may WRITE only inside the child theme. Every file tool — read, write, edit, search, index — uses the SAME path format everywhere: relative to the WordPress root, e.g. wp-content/themes/{slug}/style.css, wp-content/themes/{slug}/assets/front/css/main.css, wp-content/plugins/woocommerce/woocommerce.php, wp-includes/formatting.php. Reuse a path exactly as any tool gave it to you — never shorten, prefix-strip, or reformat it before passing it to another tool.
