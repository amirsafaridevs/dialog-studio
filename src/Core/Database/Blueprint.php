...
namespace DialogStudio\Core\Database;
...
class Blueprint
{
    ...
    public function toSql(string $charsetCollate = ''): string
    {
        ...
        foreach ($this->indexes as $keyName => $idx) {
            $cols = implode(',', array_map(
                fn (string $column): string => $this->getIndexColumnRef($column, $charsetCollate),
                $idx['columns']
            ));
    ...
    /**
     * Build index column reference with utf8mb4-safe prefix length when needed.
     *
     * @param string $columnName
     * @param string $charsetCollate
     * @return string
     */
    protected function getIndexColumnRef(string $columnName, string $charsetCollate): string
    {
        $def = $this->columns[$columnName] ?? null;
        if ($def === null || !preg_match('/varchar\((\d+)\)/', $def['type'], $matches)) {
            return $columnName;
        }

        $length = (int) $matches[1];
        $maxIndexLength = stripos($charsetCollate, 'utf8mb4') !== false ? 191 : 333;

        if ($length > $maxIndexLength) {
            return $columnName . '(' . $maxIndexLength . ')';
        }
