...
namespace DialogStudio\Core;

use DialogStudio\Contract\Interface\ApplicationInterface;
use DialogStudio\Contract\Interface\ContainerInterface;
use DialogStudio\Contract\Interface\MigrationInterface;
use DialogStudio\Contract\Interface\ServiceProviderInterface;
use DialogStudio\Contract\Interface\ServiceRegistryInterface;
...
class Application implements ApplicationInterface
{
    ...
    public function pluginActivation(): void
    {
        ...
        if ($this->textdomain !== '' && defined('DialogStudio_PLUGIN_FILE')) {
            $this->loadTextDomain();
        }

        if ($this->container->bound('dialog.runtime')) {
            $this->container->get('dialog.runtime')->activate();
        }

        if ($this->container->bound('runtime.mu_plugin')) {
            $this->container->get('runtime.mu_plugin')->activate();
        }
    }
    ...
    public function runMigrations(): void
    {
        ...
        $migrationsTable = $this->getMigrationsTableName();
        $hasTracking = DB::schema()->hasTable($migrationsTable);
        $ranMigrations = $hasTracking ? $this->getRanMigrationNames($migrationsTable) : [];
        $batch = $hasTracking ? $this->getNextMigrationBatch($migrationsTable) : 1;

        foreach ($files as $file) {
            $basename = pathinfo($file, PATHINFO_FILENAME);
            $class = $namespace . $this->resolveMigrationClassName($basename);

            if ($hasTracking && in_array($basename, $ranMigrations, true)) {
                continue;
            }
            ...
            if (DB::schema()->hasTable($migrationsTable)) {
                $this->recordMigration($migrationsTable, $basename, $batch);
                $hasTracking = true;
                $ranMigrations[] = $basename;
            }
        }
    }

    /**
     * Resolve migration class name from file basename (strip timestamp prefix).
     */
    protected function resolveMigrationClassName(string $basename): string
    {
    ...
    /**
     * Application-prefixed migrations table name (without WordPress table prefix).
     */
    protected function getMigrationsTableName(): string
    {
        return $this->prefix !== '' ? $this->prefix . '_migrations' : 'migrations';
    }

    /**
     * @return array<int, string>
     */
    protected function getRanMigrationNames(string $migrationsTable): array
    {
        $rows = DB::table($migrationsTable)->get();
        $names = [];
    ...
    protected function getNextMigrationBatch(string $migrationsTable): int
    {
        $rows = DB::table($migrationsTable)->get();
        $max = 0;
    ...
    protected function recordMigration(string $migrationsTable, string $migration, int $batch): void
    {
        DB::insert($migrationsTable, [
            'migration'  => $migration,
            'batch'      => $batch,
    ...
    public function loadTextDomain(): void
    {
        ...
        load_plugin_textdomain(
            $this->textdomain,
            false,
            dirname(plugin_basename(DialogStudio_PLUGIN_FILE)) . '/languages'
        );
    }
    /**
    ...
    {
        $viewBasePath = defined( 'DialogStudio_PLUGIN_FILE' )
            ? plugin_dir_path( DialogStudio_PLUGIN_FILE )
            : $this->basePath;
        $viewPath = $viewBasePath . 'view' . DIRECTORY_SEPARATOR . str_replace( '/', DIRECTORY_SEPARATOR, $view ) . '.php';

        if ( ! is_readable( $viewPath ) ) {
            echo '<div class="wrap"><p>' . esc_html__('View not found: ', 'DialogStudio') . esc_html($view) . '</p></div>';
            return;
        }
    ...
    {
        return plugins_url('assets/' . ltrim($src, '/'), DialogStudio_PLUGIN_FILE);
    }
