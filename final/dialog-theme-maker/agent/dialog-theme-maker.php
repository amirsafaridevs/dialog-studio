...
final class DialogStudio_Agent {
	...
	/** Slug of the WordPress theme required for Dialog Theme Maker. */
	private const REQUIRED_THEME_SLUG = 'dialog';
	...
	private function define_routes(): void {
		...
		// File Operations (restricted to dialog theme)
		...
		$this->add_route( 'POST', self::API_PREFIX . '/pages/create', 'handle_page_create' );
		$this->add_route( 'POST', self::API_PREFIX . '/pages/update', 'handle_page_update' );
	...
	public function handle_request(): void {
		...
		$api_uri = untrailingslashit( $api_uri );
	...
	private function handle_chat(): string {
		...
		$theme_status   = $this->get_dialog_theme_status();
	...
	private function get_plugin_root(): string {
		...
		return WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . 'dialog-theme-maker';
	}
	...
	private function get_plugin_file(): string {
		...
		return $this->get_plugin_root() . DIRECTORY_SEPARATOR . 'dialog-theme-maker.php';
	}
	...
	/**
	 * Dialog theme readiness for the chat UI and write operations.
	 *
	 * @return array{
	 ...
	 * }
	 */
	private function get_dialog_theme_status(): array {
		$required_slug = self::REQUIRED_THEME_SLUG;
		$dialog_theme  = wp_get_theme( $required_slug );
		$installed     = $dialog_theme->exists();
		$dialog_path   = $installed
			? wp_normalize_path( $dialog_theme->get_stylesheet_directory() )
			: wp_normalize_path( get_theme_root() . '/' . $required_slug );

		$active_theme = wp_get_theme();
		$active       = $active_theme->get_stylesheet() === $required_slug;

		return [
			'required_slug' => $required_slug,
			'installed'     => $installed,
			'active'        => $active,
			'ready'         => $installed && $active,
	...
	/**
	 * Absolute path to the dialog theme directory.
	 */
	private function get_dialog_theme_root(): string {
		$dialog_theme = wp_get_theme( self::REQUIRED_THEME_SLUG );

		if ( $dialog_theme->exists() ) {
			return wp_normalize_path( $dialog_theme->get_stylesheet_directory() );
		}

		return wp_normalize_path( get_theme_root() . '/' . self::REQUIRED_THEME_SLUG );
	}

	/**
	 * Error message when dialog theme is not ready for write operations.
	 */
	private function get_dialog_theme_write_error(): ?string {
		$status = $this->get_dialog_theme_status();
		...
		if ( ! $status['installed'] ) {
			return 'قالب dialog نصب نیست. لطفاً قالب dialog را نصب و فعال کنید.';
		}

		return sprintf(
			'قالب فعال "%s" است. برای ویرایش فایل‌ها باید قالب dialog فعال باشد.',
			$status['active_theme']['name']
		);
	}
	...
	private function is_wordpress_content_relative( string $relative ): bool {
		$content_prefixes = [ 'plugins/', 'themes/', 'mu-plugins/', 'uploads/' ];
	...
	private function resolve_read_path( string $relative ): string {
		$dialog_path = $this->get_dialog_theme_root();
	...
	private function get_allowed_paths( string $operation ): array {
		$dialog_theme_path = $this->get_dialog_theme_root();
	...
	/**
	 * Verify the dialog theme directory exists on disk.
	 *
	 * @return array{exists: bool, path: string, error: string|null}
	 */
	private function get_dialog_theme_info(): array {
		$dialog_path = $this->get_dialog_theme_root();
		$status      = $this->get_dialog_theme_status();
		...
		if ( ! is_dir( $dialog_path ) ) {
			return [
				'exists' => false,
				'path'   => $dialog_path,
				'error'  => 'پوشه قالب dialog یافت نشد.',
			];
		}
	...
	private function handle_file_write(): array {
		...
		$theme_error = $this->get_dialog_theme_write_error();
		if ( $theme_error !== null ) {
			return [
				'success' => false,
				'data' => null,
	...
	private function handle_file_list(): array {
		...
		if ( empty( $path ) ) {
			$path = $this->get_dialog_theme_root();
			if ( ! is_dir( $path ) ) {
				return [
					'success' => false,
					'data' => null,
	...
	private function handle_search_files(): array {
		...
		// Default to entire WordPress install if no directory provided
		if ( empty( $directory ) ) {
			$directory = $this->get_default_read_scope();
		}
	...
	private function handle_search_content(): array {
		...
		if ( empty( $path ) ) {
			$path = $this->get_dialog_theme_relative_scope();
		}
	...
	private function handle_theme_check(): array {
		...
		$status      = $this->get_dialog_theme_status();
		$theme_info  = $this->get_dialog_theme_info();
		$active_theme = wp_get_theme();

		$message = null;
		if ( ! $status['ready'] ) {
			if ( ! $status['installed'] ) {
				$message = 'قالب dialog نصب نیست. لطفاً قالب dialog را نصب و فعال کنید.';
			} else {
				$message = sprintf(
		...
		return [
			'success' => true,
			'data' => [
				'required_theme' => self::REQUIRED_THEME_SLUG,
				'ready' => $status['ready'],
	...
	private function handle_theme_create(): array {
		...
		return [
			'success' => false,
			'data' => null,
			'error' => 'قالب dialog باید از بخش ظاهر → قالب‌ها در وردپرس نصب و فعال شود.',
		];
	}

	private function get_dialog_theme_relative_scope(): string {
		$dialog_root = $this->get_dialog_theme_root();
		$wp_root     = $this->get_wordpress_root();

		if ( str_starts_with( $dialog_root, $wp_root ) ) {
			return ltrim( substr( $dialog_root, strlen( $wp_root ) ), '/' );
		}

		return 'wp-content/themes/' . self::REQUIRED_THEME_SLUG;
	}
	...
	private function handle_code_graph(): array {
		...
		if ( '' === $directory ) {
			$directory = $this->get_dialog_theme_relative_scope();
		}
	 ...
	 *
	 * Builds a compact code index (PHP, CSS, JS) for the active theme.
	...
	private function handle_theme_index(): array {
		...
		try {
			...
			$index = $indexer->buildCompactIndex();
	...
	/**
	 * POST /DialogStudio/v1/pages/create
	 */
	private function handle_page_create(): array {
		$this->require_chat_user();

		$permission_error = $this->require_manage_pages_permission();
		if ( $permission_error ) {
		...
		$post_data = [
			...
			'post_status'  => $this->sanitize_page_status( (string) ( $body['status'] ?? 'publish' ) ),
		...
		$this->apply_page_extras( (int) $post_id, $body );

		return [
			'success' => true,
			'data'    => $this->format_page_response( (int) $post_id ),
			'error'   => null,
		];
	}

	/**
	 * POST /DialogStudio/v1/pages/update
	 */
	private function handle_page_update(): array {
		$this->require_chat_user();
		...
		$body    = $this->get_json_body();
		$page_id = $this->resolve_page_id( $body );
		if ( $page_id <= 0 ) {
			return [
				'success' => false,
				'data'    => null,
		...
		if ( array_key_exists( 'status', $body ) ) {
			$post_data['post_status'] = $this->sanitize_page_status( (string) $body['status'] );
		}
		...
		$this->apply_page_extras( $page_id, $body );

		return [
			'success' => true,
			'data'    => $this->format_page_response( $page_id ),
			'error'   => null,
		];
	}
	 ...
	 */
	private function require_manage_pages_permission(): ?array {
		$settings = $this->get_settings();
		if ( empty( $settings['permissions']['manage_pages'] ) ) {
			return [
				'success' => false,
				'data'    => null,
				'error'   => 'Page management permission denied',
			];
		}
	...
	private function sanitize_page_status( string $status ): string {
	 ...
	 */
	private function resolve_page_id( array $body ): int {
		if ( ! empty( $body['id'] ) ) {
	 ...
	 * @param array<string, mixed> $body
	 */
	private function apply_page_extras( int $page_id, array $body ): void {
	 ...
	 */
	private function format_page_response( int $page_id ): array {
		$post = get_post( $page_id );
		if ( ! $post ) {
			return [ 'id' => $page_id ];
		}
